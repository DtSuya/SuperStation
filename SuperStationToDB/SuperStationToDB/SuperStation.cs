...
namespace Utility.SuperStation
{
    public class ExternalData
    {
        ...
        public ExternalData(Database Database)
        {
            m_Database = Database;
        ...
        public static DataTable CalAOD(double v, double Latitude, DateTime time, DataTable sources)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    //2016年后的压强数据
                    //m_P = Convert.ToDouble(m_Database.GetFirstValue("select Value from T_SuperStation where StartTime='" + time.Date + "' and Item=28"));
                    //2016年前只有整天的压强数据
                    //bool t = Double.TryParse(m_Database.GetFirstValue("select Value from T_SuperStation where StartTime='" + hourtime + "' and Item=28"), out m_P);
                    //if (t == false)
                    m_P = Double.Parse(m_Database.GetFirstValue("select Value from T_SuperStation where StartTime='" + time.Date.AddHours(time.Hour ) + "' and Item=28"));
                    m_Theta = Double.Parse(m_Database.GetFirstValue("select avg( DValue) from T_SuperStationmin where  CONVERT(varchar(100), StartTime, 23)='" + time.Date.ToString("yyyy-MM-dd") + "'   and Item=360 and DValue!=-99 "));
                }
                catch
                {
                    //加载数据
                ...
                try
                {
                    m_U = Convert.ToDouble(m_Database.GetFirstValue("select avg( DValue) from T_SuperStationmin where  CONVERT(varchar(100), StartTime, 23)='" + time.Date.ToString("yyyy-MM-dd") + "'   and Item=359 and DValue!=-99 "));
                }
                catch
                {
                    m_U = 302.2000;
                ...
                double  dn440=Convert.ToDouble(drsource["value"]);
                drsource = sources.Select("parameterid =112")[0];
                double dn870 = Convert.ToDouble(drsource["value"]);
                if  (dn440==0||dn870==0)
                 return dtAOD;
                double a_aod = SpecialAOD(440, dn440, time, waveTable);
                //870
               
                double b_aod = SpecialAOD(870, dn870, time, waveTable);
        ...
        public static DataTable MonthColumnO3toDb(string filepath, string type, string station, string city)
        {
        ...
        public static DataTable O3Lidar(DataTable o3leida1,DateTime lst,string type)
        {
